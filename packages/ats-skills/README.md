...
`initializeMemory({agentId,directory,sizeGb,python?,bootstrap?,ownerScope?,signal?})` verifies the actual `aether-context==0.3.1` backend, available space and a 5–1024 GiB configured ceiling. Unsupported engine versions report unavailable before creating storage. A new pool is built in a journal-owned staging directory on the chosen drive, writes and reopens a verification slice, removes the slice, publishes native files exclusively, and reopens a digest-checked snapshot of those files before committing a ready binding. Interrupted publication resumes only when journal identity and file digests still agree. A separate scope-bound verification journal cleans up this setup's private snapshot after process death; unknown snapshot contents are preserved and refused. The selected filesystem must support atomic hard links; unsupported drives fail with an actionable message and preserve pending setup files. Unknown or changed files are never overwritten by recovery.

Existing memory is reverified by reopening a digest-checked private copy with the native engine, preserving original vectors and metadata even at capacity. This needs a writable parent directory and enough free disk for a temporary snapshot of the actual native files; it does not reserve the full ceiling twice. Receipts distinguish `native_pool_init_and_snapshot_reopen` from `persisted_snapshot_reopen`, retain the canonical location/runtime version, and report `quota_kind: native_slice_accounting` and `reserved_bytes: 0`. The configured ceiling is not a reserved allocation or hard filesystem quota; the flat index also has RAM costs.
...
Run `npm test` for bridge, observer and settings tests. Set `AETHER_ATS_REQUIRE_NATIVE=1`, `AETHER_ATS_TEST_PYTHON` and `AETHER_ATS_RUNTIME_PATH` for qualification: missing native memory/compiler dependencies then fail instead of skipping. The required Electron verify gate depends on the dedicated Node/native package workflow with pinned Context/Nano dependencies; these Node tests are excluded from Vitest only because that replacement gate runs them. Linux fixture tests are not Windows filesystem/process qualification or a hosted CI result; those receipts remain separate. Verify clean installation from a packed Agent artifact before releasing a dependency bump. Nothing in this package publishes a release automatically.
