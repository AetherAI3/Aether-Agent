...
async function loadPackage(): Promise<AtsPackage> {
  ...
  for (const key of ["initializeMemory", "scanStrategies", "createBrowserObserver", "observeBrowser", "BrowserSessionRecovery", "loadSettings", "saveSettings", "cyclePermissionMode", "dataStreamStatus"] as const) {
...
export function createAtsHooks(deps: AtsHookDeps = {}): ManagedAgentHooks {
  ...
  const chatOwners = new WeakMap<ManagedChatSurface, { account: ManagedAccountScope; agentId: string; closed: boolean; cleanup(): Promise<void> }>();
  ...
  const initialize = async (account: ManagedAccountScope, agent: ManagedAgent, options: Awaited<ReturnType<typeof askSetup>>, pack: AtsPackage, write = output, signal?: AbortSignal): Promise<Binding> => {
    ...
    const scan = await pack.scanStrategies({ directory: binding.strategies_directory, signal,
      ...(env["AETHER_ATS_PYTHON"] ? { python: env["AETHER_ATS_PYTHON"] } : {}) });
    signal?.throwIfAborted();
  ...
  return {
    ...
    onChatCommand: async (ctx, agent, input, surface) => {
      const output = surface?.write ?? deps.output ?? ((text: string) => process.stdout.write(text));
      if (/^\/(?:browser|ats\s+browser)(?:\s|$)/.test(input)) return browser(await accountForChat(ctx, agent, surface), agent, surface).command(input);
    ...
    beforeChat: async (ctx, agent, surface) => {
      ...
      try {
        ...
        surface?.setContext?.({ memory: `verified ${binding.memory_gb} GiB`, mode: `${settings.permission_mode} requested`, strategies: "configured", data: String(pack.dataStreamStatus(settings)["state"] ?? "unverified") });
