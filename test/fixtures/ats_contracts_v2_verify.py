...
An independent Python mirror of the Spec 1 validators at /1 and /2 and of the
executable chain gate (src/core/ats_contracts/{proposal,order,approval,grant,
connector,mode}.ts), plus a harness that runs every vector in
ats_contracts_v2_golden.json and requires byte-identical digests and
identical refusal messages; see docs/CONTRACTS.md, "Spec 1 /2 closure".

It imports nothing from TypeScript. It borrows the primitives and the two
strict helpers from ats_browser_order_wire.py and the RFC 8785 encoder from
ats_contracts_golden_verify.py (both independent mirrors themselves), so each
rule has one Python definition, just as each has one TypeScript definition.
ATSv2 lifts the mirror half (everything above the harness).

Cross-language trap handled here on purpose: Python's \\d matches every
Unicode digit, JavaScript's only ASCII 0-9. The frozen /1 label rule is
written [0-9]{5,}; with \\d the Python /1 mirror would refuse the
fullwidth-digit weakness that TypeScript /1 accepts, and the fixture's
frozen_weakness section would stop proving anything about /1.
"""
...
def check_documents(fixture: dict[str, Any]) -> list[str]:
    failures: list[str] = []
...
def check_fixture(fixture: dict[str, Any]) -> list[str]:
    ...
    failures += check_documents(fixture)
